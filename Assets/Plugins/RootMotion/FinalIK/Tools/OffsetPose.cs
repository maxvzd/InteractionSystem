...
namespace RootMotion.FinalIK
{
    ...
    public class OffsetPose : MonoBehaviour
    {
        ...
        public class EffectorLink
        {
            ...
            public void Apply(IKSolverFullBodyBiped solver, float weight, Quaternion rotation, Vector3 relativeTo)
            {
                // Offset
        ...
        {
            for (int i = 0; i < effectorLinks.Length; i++) effectorLinks[i].Apply(solver, weight, solver.GetRoot().rotation, solver.GetRoot().position);
        }

        public void Apply(IKSolverFullBodyBiped solver, float weight, Quaternion rotation, Vector3 relativeTo)
        {
            for (int i = 0; i < effectorLinks.Length; i++) effectorLinks[i].Apply(solver, weight, rotation, relativeTo);
        }
    }
}